...
// ─── Mode select ──────────────────────────────────────────────────────────────
const modeDescs = {
  '1v1': '1 player each side',
  '2v1': '2 on the left, 1 on the right',
  '1v2': '1 on the left, 2 on the right',
  '2v2': '2 players each side',
};
...
socket.on('playerJoined', ({ playerId, state }) => {
  gameState = state;
  updateSlotsUI(state);
});
...
socket.on('playerLeft', ({ playerId }) => {
  ...
  // If in game, show disconnect notice
...
socket.on('gameover', ({ score }) => {
  stopInputLoop();
  let result;
  if (score.left > score.right)       result = '🏆 Left Team Wins!';
  else if (score.right > score.left)  result = '🏆 Right Team Wins!';
  else                                result = "It's a Draw!";
  fulltimeResult.textContent = result;
...
function updateSlotsUI(state) {
  ...
  function renderTeam(list, teamName) {
    return `
      <div class="slots-team">
        <div class="slots-team-label">${teamName}</div>
        ${list.map(p => {
          const isYou    = p.id === myPlayerId;
          const filled   = p.connected;
          const dotColor = p.color;
          return `
            <div class="slot-card ${filled ? 'filled' : ''} ${isYou ? 'you' : ''}" style="--dot-color:${dotColor}">
              <span class="slot-dot"></span>
              <span>${filled ? (p.label + (isYou ? ' (You)' : '')) : 'Waiting…'}</span>
            </div>`;
        }).join('')}
      </div>`;
  }
  ...
  slotsGrid.innerHTML = `
    ${renderTeam(leftPlayers, '⬅ Left Team')}
    <div class="vs-divider">VS</div>
    ${renderTeam(rightPlayers, 'Right Team ➡')}
  `;
}
...
function showGame(state) {
  ...
  // Show your player's team color in the hint
  const me = state.players[myPlayerId];
  if (me) {
    yourHint.textContent  = `You are ${me.team === 'left' ? '⬅ Left' : '➡ Right'} Team`;
...
function renderLoop() {
  ...
  if (gameState) {
    drawField();
    Object.values(gameState.players).forEach(p => drawPlayer(p));
...
function drawField() {
  ...
  [
    [FIELD.x,           FIELD.y,           0            ],
    [FIELD.x+FIELD.w,   FIELD.y,           Math.PI/2    ],
    [FIELD.x+FIELD.w,   FIELD.y+FIELD.h,   Math.PI      ],
    [FIELD.x,           FIELD.y+FIELD.h,   Math.PI*1.5  ]
  ].forEach(([cx,cy,sa]) => {
    ctx.beginPath(); ctx.arc(cx,cy,16,sa,sa+Math.PI/2);
...
function drawPlayer(p) {
  ...
  ctx.beginPath();
  ctx.ellipse(p.x+3, p.y+p.r+2, p.r*0.8, 5, 0, 0, Math.PI*2);
  ctx.fill();

  // Outer ring — gold for yourself, white for others
  ctx.beginPath();
  ...
  // Shine
  ctx.beginPath();
  ctx.arc(p.x-p.r*0.2, p.y-p.r*0.2, p.r*0.38, 0, Math.PI*2);
  ctx.fillStyle = 'rgba(255,255,255,0.2)';
  ctx.fill();

  // Label (name / number)
  ctx.fillStyle    = '#fff';
  ctx.font         = `bold ${Math.floor(p.r*0.75)}px Barlow, sans-serif`;
  ctx.textAlign    = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(p.label, p.x, p.y+1);

  // "YOU" indicator arc above
  if (isMe) {
    ctx.beginPath();
    ctx.arc(p.x, p.y, p.r+5, Math.PI*1.2, Math.PI*1.8);
...
function drawBall(b) {
  ...
  ctx.fillStyle = 'rgba(0,0,0,0.25)';
  ctx.beginPath();
  ctx.ellipse(b.x+3, b.y+b.r-1, b.r*0.8, 4, 0, 0, Math.PI*2);
  ctx.fill();

  ctx.beginPath();
  ctx.arc(b.x, b.y, b.r, 0, Math.PI*2);
  const bg = ctx.createRadialGradient(b.x-3, b.y-3, 1, b.x, b.y, b.r);
  bg.addColorStop(0, '#ffffff'); bg.addColorStop(0.6, '#e8e8e8'); bg.addColorStop(1, '#aaaaaa');
