...
// Modes: 1v1 = 1 per team, 2v2 = 2 per team (host can start 2v2 with 3 players — empty slot is fine)
const MODE_SLOTS = {
  '1v1': { left: 1, right: 1 },
  '2v2': { left: 2, right: 2 },
};
...
function slotStartX(side, idx, total) {
  if (side === 'left')  return total === 1 ? FIELD.x + 60  : FIELD.x + 50 + idx * 30;
  else                  return total === 1 ? FIELD.x + FIELD.w - 60 : FIELD.x + FIELD.w - 50 - idx * 30;
}
...
function makeGameState(mode) {
  ...
  for (let i = 0; i < left; i++) {
    ...
    players[id] = {
      ...
      x: slotStartX('left', i, left), y: slotStartY(i, left),
      r: PLAYER_R, color: TEAM_COLORS.left[i], label: String(i + 1),
      socketId: null, keys: {}
    };
  }
  for (let i = 0; i < right; i++) {
    ...
    players[id] = {
      ...
      x: slotStartX('right', i, right), y: slotStartY(i, right),
      r: PLAYER_R, color: TEAM_COLORS.right[i], label: String(i + 1),
      socketId: null, keys: {}
    };
  }

  return {
    players, ball: makeBall(),
    score: { left: 0, right: 0 },
    timeLeft: MATCH_DUR, frameCount: 0,
    phase: 'waiting', mode
  };
}
...
function tick(code) {
  ...
  if (inGoal) {
    if (b.x - b.r < FIELD.x - FIELD.goalW) {
      ...
      st.phase = 'cooldown';
      setTimeout(() => { resetPositions(st); st.phase = 'playing'; io.to(code).emit('reset', sanitize(st)); }, 1500);
      return;
    }
  ...
  players.forEach(p => {
    ...
    if (dist < minD && dist > 0) {
      ...
      b.vx = nx * Math.max(spd, KICK_FORCE);
      b.vy = ny * Math.max(spd, KICK_FORCE);
    }
  });

  // Broadcast
  const bp = {};
  players.forEach(p => { bp[p.id] = { id:p.id, x:p.x, y:p.y, r:p.r, color:p.color, label:p.label, team:p.team }; });
  io.to(code).emit('state', { players: bp, ball: st.ball, timeLeft: st.timeLeft, score: st.score });
}
...
io.on('connection', socket => {
  ...
  socket.on('createRoom', ({ mode, nickname }) => {
    ...
    firstSlot.label    = (nickname || 'P1').substring(0, 6);
  ...
  socket.on('joinRoom', ({ code, nickname }) => {
    ...
    if (room.state.phase === 'gameover'){ socket.emit('joinError', 'Game already over');   return; }

    const emptySlot = Object.values(room.state.players).find(p => !p.socketId);
    if (!emptySlot)                     { socket.emit('joinError', 'Room is full');         return; }

    emptySlot.socketId = socket.id;
    emptySlot.label    = (nickname || 'P?').substring(0, 6);
  ...
  socket.on('startGame', ({ code }) => {
    const room = rooms[code];
    if (!room || room.hostId !== socket.id || room.state.phase !== 'waiting') return;
  ...
  socket.on('disconnect', () => {
    for (const code of Object.keys(rooms)) {
      ...
      io.to(code).emit('playerLeft', { playerId: pid });
